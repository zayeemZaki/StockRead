#!/usr/bin/env python3
"""Market Maker - S&P 500 Stock Price Fetcher"""
...
# Fetch the full S&P 500 ticker list dynamically
TICKERS = get_sp500_tickers()


def fetch_all_stock_prices() -> list:
        ...
        try:
            ...
            if len(chunk_tickers) == 1:
                ...
                if not df.empty and 'Close' in df.columns:
                    process_single_ticker(ticker, df, stock_data_list)
                    chunk_success += 1
            else:
                for ticker in chunk_tickers:
                    try:
                        ...
                        ticker_df = df[ticker]
                        process_single_ticker(ticker, ticker_df, stock_data_list)
                        chunk_success += 1
                    except Exception as e:
...
def process_single_ticker(ticker: str, ticker_df: pd.DataFrame, stock_data_list: list):
...
def save_bulk_to_supabase(stock_data_list: list) -> int:
    ...
    for i in range(0, len(stock_data_list), db_chunk_size):
        ...
        try:
            supabase.table('market_prices').upsert(chunk, on_conflict='symbol').execute()
            saved += len(chunk)
        except Exception as chunk_error:
...
def update_market():
    ...
    stock_data = fetch_all_stock_prices()
    
    if not stock_data:
        logger.warning("No stock data retrieved, skipping database update")
        return
    
    saved_count = save_bulk_to_supabase(stock_data)
...
def run_market_maker():
    ...
    logger.info(f"Market Maker started - tracking {len(TICKERS)} stocks, update interval: 5 minutes")
...
if __name__ == "__main__":
    ...
    try:
        supabase.table('market_prices').select('symbol').limit(1).execute()
